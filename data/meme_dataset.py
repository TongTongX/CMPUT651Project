...
class MemeDataset(Dataset):
    ...
    def _preprocess_dataset(self):
        ...
        invalid_row_indices = self.meme_frame[
            ...
            | ~self.meme_frame['overall_sentiment'].isin(VALID_LABELS['overall_sentiment'])
        ...
        self.meme_frame['ocr_extracted_text'] = (self.meme_frame['ocr_extracted_text'].astype('str'))
        self.meme_frame['ocr_extracted_text'] = (self.meme_frame['ocr_extracted_text'].str.lower())
        self.meme_frame['corrected_text'] = (self.meme_frame['corrected_text'].astype('str'))
        self.meme_frame['corrected_text'] = (self.meme_frame['corrected_text'].str.lower())
        ...
        for label in VALID_LABELS.keys():
            onehot_array = pd.get_dummies(data=self.meme_frame[label])[VALID_LABELS[label]].values
    ...
    def __getitem__(self, idx):
        ...
        if not self.is_test:
            humour_onehot = torch.from_numpy(np.array(self.meme_frame['humour_onehot'][idx])).unsqueeze_(0)
            sarcasm_onehot = torch.from_numpy(np.array(self.meme_frame['sarcasm_onehot'][idx])).unsqueeze_(0)
            offensive_onehot = torch.from_numpy(np.array(self.meme_frame['offensive_onehot'][idx])).unsqueeze_(0)
            motivational_onehot = torch.from_numpy(np.array(self.meme_frame['motivational_onehot'][idx])).unsqueeze_(0)
            overall_sentiment_onehot = (torch.from_numpy(np.array(self.meme_frame['overall_sentiment_onehot'][idx])).unsqueeze_(0))
